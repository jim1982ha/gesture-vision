...
export class PluginUIService {
  ...
  private injectStylesheet(pluginId: string, manifest: PluginManifest): void {
    ...
    const link = Object.assign(document.createElement('link'), {
        ...
        // FIXED: Relative path for Ingress + cache buster
    ...
    const loadPromise = (async () => {
        ...
        if (modulePath) {
             ...
             if (import.meta.env.DEV) {
                 ...
                 try {
                    // FIX: Added timestamp to bypass browser cache for newly installed plugins
                    const loaded = await import(/* @vite-ignore */ `plugins/${pluginId}/frontend/index.js?t=${Date.now()}`);
                    module = loaded.default;
                 } catch (e) {
