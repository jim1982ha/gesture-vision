...
export class CustomGesturesTab extends BaseSettingsTab<CustomGesturesTabElements> {
    ...
    constructor(appStore: AppStore, uiControllerRef: UIController, elementQueries: { [K in keyof CustomGesturesTabElements]: string }) {
    ...
    protected _doesConfigUpdateAffectThisTab(): boolean { return false; }
    ...
    #parseMetadataFromCodeString(codeString: string): Omit<StagedImportData, 'codeString'> | null {
        ...
        try {
            ...
            return { name: metadata.name.trim(), description: typeof metadata.description === 'string' ? metadata.description.trim() : '', type: metadata.type, };
        } catch { return null; }
    }
    ...
    #performActualUpload = (): void => {
        ...
        const payloadToSend: UploadCustomGesturePayload = { name: finalName, description: finalDescription, type: this.#stagedForImport.type, codeString: this.#stagedForImport.codeString, source: 'core' };
    ...
    #handleDeleteAck = (payload: DeleteCustomGestureAckPayload): void => { if (payload.success) { this.#editableCardInstances.delete(payload.deletedId || ''); pubsub.publish(UI_EVENTS.SHOW_NOTIFICATION, { messageKey: 'customGestureDeleteSuccess', substitutions: { id: payload.deletedId ?? 'N/A' }, type: 'info' }); } else pubsub.publish(UI_EVENTS.SHOW_ERROR, { messageKey: 'customGestureDeleteFailed', substitutions: { message: payload.message || 'Unknown error' } }); };
    ...
    #renderCustomGestureList = (definitions: CustomGestureMetadata[] = []): void => {
        ...
        this.#editableCardInstances.clear(); hc.innerHTML = ''; pc.innerHTML = '';
        const hands = definitions.filter(d => d.type !== 'pose'); const poses = definitions.filter(d => d.type === 'pose');
        if (hands.length === 0) { hp.textContent = translate('noCustomGesturesSaved', { type: translate('Hand') }); setElementVisibility(hp, true, 'block'); } else { setElementVisibility(hp, false); hands.forEach(d => hc.appendChild(this.#createGestureListItem(d))); }
        if (poses.length === 0) { pp.textContent = translate('noCustomGesturesSaved', { type: translate('Pose') }); setElementVisibility(pp, true, 'block'); } else { setElementVisibility(pp, false); poses.forEach(d => pc.appendChild(this.#createGestureListItem(d))); }
    }

    #createGestureListItem = (def: CustomGestureMetadata): HTMLDivElement => {
        const cardId = `custom-gesture-card-${def.id}`;
        
        const saveButton = createCardActionButton({ action: 'save', titleKey: 'saveTooltip', iconKey: 'UI_SAVE', textKey: 'save', extraClasses: ['btn-primary', 'save-btn'] });
        const cancelButton = createCardActionButton({ action: 'cancel', titleKey: 'cancelTooltip', iconKey: 'UI_CANCEL', textKey: 'cancel', extraClasses: ['btn-secondary', 'cancel-btn'] });
        
        const detailsHtml = `<div class="plugin-view-content-wrapper"></div><form class="plugin-global-settings-form hidden" onsubmit="return false;"><div class="form-fields-container"></div><div class="mt-4 flex justify-end gap-2">${cancelButton.outerHTML}${saveButton.outerHTML}</div></form>`;
        const deleteButton = createCardActionButton({ action: 'delete', titleKey: 'deleteTooltip', iconKey: 'UI_DELETE', extraClasses: ['btn-icon-danger', 'delete-btn'], });
        
        const card = createCardElement({ ...getGestureCategoryIconDetails(def.type === 'pose' ? 'CUSTOM_POSE' : 'CUSTOM_HAND'), title: def.name, actionButtonsHtml: deleteButton.outerHTML, detailsHtml, itemClasses: "config-item card-item-clickable", datasetAttributes: { gestureId: def.id, gestureName: def.name }, titleAttribute: translate('editTooltip', { item: def.name }), ariaLabel: translate('editTooltip', { item: def.name }) });
        card.id = cardId;

        const viewWrapper = card.querySelector('.plugin-view-content-wrapper') as HTMLDivElement;
        const form = card.querySelector('.plugin-global-settings-form') as HTMLFormElement;
        const formFields = form.querySelector('.form-fields-container') as HTMLFormElement;
        
        viewWrapper.innerHTML = `<div class="card-details"><div class="card-detail-line"><span class="material-icons card-detail-icon"></span><span class="card-detail-value allow-wrap">${def.description || ''}</span></div></div>`;
        setIcon(viewWrapper.querySelector('.card-detail-icon'), 'UI_NOTES');

        formFields.innerHTML = `<div class="form-group"><label for="${cardId}-name" class="form-label">${translate('nameLabel')}</label><input type="text" id="${cardId}-name" class="form-control" value="${def.name}"></div><div class="form-group"><label for="${cardId}-desc" class="form-label">${translate('descriptionOptionalLabel')}</label><textarea id="${cardId}-desc" class="form-control" rows="2">${def.description || ''}</textarea></div>`;
        ...
        this.#editableCardInstances.set(def.id, new EditableCard({ cardElement: card, viewElementsContainer: viewWrapper, formElement: form, saveButton: saveBtn, cancelButton: cancelBtn, onSave: async () => { const newName = (card.querySelector<HTMLInputElement>(`#${cardId}-name`)?.value || '').trim(); const newDescription = (card.querySelector<HTMLTextAreaElement>(`#${cardId}-desc`)?.value || '').trim(); if (!newName) { pubsub.publish(UI_EVENTS.SHOW_ERROR, { messageKey: "customGestureNameReq" }); return false; } const payload: UpdateCustomGesturePayload = { id: def.id, oldName: def.name, newName, newDescription }; const result = await webSocketService.request<UpdateCustomGestureAckPayload>(WEBSOCKET_EVENTS.UPDATE_CUSTOM_GESTURE, payload); return result.success; }, onCancel: () => { const nameInput = card.querySelector<HTMLInputElement>(`#${cardId}-name`); const descTextarea = card.querySelector<HTMLTextAreaElement>(`#${cardId}-desc`); if (nameInput) nameInput.value = def.name; if (descTextarea) descTextarea.value = def.description || ''; } }));
