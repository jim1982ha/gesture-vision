#!/bin/bash
# Usage: ./tools/release.sh -t patch -m "Commit message" -c "Changelog message"
cd "$(dirname "$0")/.." || exit 1
...
display_help() {
    ...
    echo "Automates version bumping, changelog updating, and git release commits."
    echo
    ...
    echo
    echo "Examples:"
    echo "  1. Standard patch release:"
    echo "     $0 -t patch -m \"Fix navigation bug\""
    echo
    echo "  2. Release with detailed multi-line changelog:"
    echo "     $0 -t minor -m \"feat: New Dashboard\" -c \"- Added Dashboard\n- Fixed Nginx buffers\""
    echo
}
...
if [ "$SKIP_BUMP" = false ]; then
    ...
    # If -c was provided, use it. Otherwise fallback to commit message.
    LOG_TEXT=""
